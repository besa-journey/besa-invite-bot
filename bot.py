...
async def tours(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for tour in all_tours:
        tour_id, title, desc, location, date, capacity, price, is_active, created_by, created_at = tour
        text += f"━━━━━━━━━━━━━━━━━━━━\n🎯 *{title}*\n📍 مکان: {location or 'نامشخص'}\n📅 تاریخ: {date or 'نامشخص'}\n👥 ظرفیت: {capacity} نفر\n🆔 کد تور: `{tour_id}`\n"
    text += "\n━━━━━━━━━━━━━━━━━━━━\nبرای ثبت‌نام: `/register <کد تور>`"
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def newtour(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 4:
        await update.message.reply_text("📝 استفاده:\n`/newtour عنوان | مکان | تاریخ | ظرفیت | توضیحات`", parse_mode="Markdown")
        return
    full_text = " ".join(context.args)
    parts = [p.strip() for p in full_text.split("|")]
    title, location, date = parts[0], parts[1], parts[2]
    capacity = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 0
    description = parts[4] if len(parts) > 4 else ""
    create_tour(title, description, location, date, capacity, 0, update.effective_user.id)
    await update.message.reply_text(f"✅ تور «{title}» ساخته شد!\n\nبرای دیدن: /tours")
...
async def polls(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for poll in active_polls:
        ...
        for i, opt in enumerate(options, 1):
            text += f"  {i}. {opt}\n"
    text += "\n━━━━━━━━━━━━━━━━━━━━\nبرای رأی: `/vote <شماره نظرسنجی> <شماره گزینه>`"
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def newpoll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        ...
        return
    if len(context.args) < 3:
        await update.message.reply_text("📝 استفاده:\n`/newpoll سوال | گزینه1 | گزینه2 | گزینه3`", parse_mode="Markdown")
        return
    full_text = " ".join(context.args)
    parts = [p.strip() for p in full_text.split("|")]
    question = parts[0]
